  ...
  return (
    <div className={`relative flex gap-8 ${isLocked ? 'opacity-60' : ''}`}>
      ...
      <div className="flex flex-col items-center shrink-0">
        <div className={`w-3 h-3 rounded-full shadow-lg z-10 mt-10 ring-4 ring-background ${isActive ? 'bg-primary shadow-primary/40' : isInProgress ? 'bg-cyan-400 shadow-cyan-400/40' : 'bg-outline-variant/30'}`} />
        {index < 2 && <div className="w-[2px] flex-1 mt-1" style={{ background: isActive ? 'linear-gradient(to bottom, #6366f1, rgba(99,102,241,0.1))' : 'rgba(139,148,158,0.1)' }} />}
      </div>

      {/* Card */}
      <div className={`flex-1 mb-16 bg-surface rounded-2xl border shadow-xl overflow-hidden transition-colors duration-300 ${isActive ? 'border-primary/20' : 'border-outline-variant/20'}`}>
        <div className="p-8">
          <div className="flex items-start justify-between mb-4">
            <div>
              <div className="text-[10px] font-black uppercase tracking-[0.3em] mb-1" style={{ color: step.color }}>Step {step.num}</div>
              <h3 className="text-2xl font-black text-on-background">{step.title}</h3>
            </div>
            {isActive && <Check size={20} className="text-primary mt-1 shrink-0" />}
            {isLocked && <Lock size={18} className="text-on-surface-variant mt-1 shrink-0" />}
          </div>

          <p className="text-sm text-on-surface-variant leading-relaxed mb-5">{step.desc}</p>

          {/* Tags */}
          {step.tags?.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-5">
              {step.tags.map((tag, i) => (
                <span key={i} className="px-3 py-1 rounded-md bg-outline-variant/10 border border-outline-variant/30 text-[11px] font-bold text-on-background/60">{tag}</span>
              ))}
            </div>
          )}
          ...
          {isInProgress && (
            <div className="mb-5">
              <div className="flex justify-between items-center mb-2">
                <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Current Progress</span>
              ...
              <div className="grid grid-cols-3 gap-3 mt-4">
                {['Project 1', 'Project 2', 'Project 3'].map((p, i) => (
                  <div key={i} className={`p-3 rounded-xl border text-center text-[10px] font-black uppercase tracking-widest ${i === 0 ? 'bg-primary/20 border-primary/30 text-primary' : i === 1 ? 'bg-cyan-400/10 border-cyan-400/20 text-cyan-400' : 'bg-outline-variant/5 border-outline-variant/20 text-on-surface-variant/50'}`}>
          ...
          {/* Locked Note */}
          {isLocked && step.note && (
            <p className="text-xs text-on-surface-variant italic leading-relaxed mb-5 border-l-2 border-outline-variant/30 pl-4">"{step.note}"</p>
          )}
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-3 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${
              isLocked
                ? 'bg-outline-variant/10 text-on-surface-variant/50 cursor-not-allowed border border-outline-variant/20'
                : isActive
    ...
    return (
      <div className="flex h-screen bg-background transition-colors duration-300 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-10">
          ...
          <h2 className="text-2xl font-black text-on-background mb-3">No Roadmap Generated</h2>
          <p className="text-on-surface-variant text-sm mb-8 text-center max-w-xs">Complete your career analysis to generate your personalized learning roadmap.</p>
      ...
      <div className="flex-1 flex flex-col h-full overflow-y-auto custom-scrollbar">
        ...
        <main className="flex-1 p-10 lg:p-12">
          ...
          <div className="mb-10 max-w-2xl">
            ...
            <h1 className="text-4xl font-black text-on-background mb-3">CareerCraft <span className="text-on-background/40">Roadmap</span></h1>
            <p className="text-sm text-on-surface-variant leading-relaxed">
              Your AI-generated vertical progression path for <span className="text-on-background font-bold">{mainRole}</span>. Each step is engineered to maximize your market value through rigorous skill acquisition.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-5 space-y-6">
              {/* AI Guidance */}
              <div className="bg-surface rounded-2xl p-7 border border-primary/20 shadow-xl transition-colors duration-300">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-8 h-8 rounded-xl bg-primary/20 border border-primary/30 flex items-center justify-center">
                    <Zap size={14} className="text-primary animate-pulse" />
                  </div>
                  <div className="text-[10px] font-black text-on-background uppercase tracking-widest">Next Best Action</div>
                </div>
                <p className="text-sm text-on-surface-variant italic leading-relaxed">
                  "Focus on mastering <span className="text-on-background font-bold">{missing[0] || 'your core skills'}</span> to unlock Step 02. Your profile shows strong potential for growth in the <span className="text-on-background font-bold">{mainRole}</span> track."
                </p>
              </div>

              {/* Course Recommendations */}
              {courses.length > 0 && (
                <div className="bg-surface rounded-2xl p-7 border border-outline-variant/20 shadow-xl transition-colors duration-300">
                  <div className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest mb-4">Recommended Courses</div>
                  <div className="space-y-3">
                    {courses.slice(0, 3).map((item, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3 p-3 rounded-xl bg-outline-variant/10 border border-outline-variant/20 hover:border-primary/40 transition-all cursor-pointer group">
                          <div className="w-6 h-6 rounded-lg bg-primary/10 flex items-center justify-center shrink-0">
                            <span className="text-[10px] font-black text-primary">{i + 1}</span>
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="text-xs font-bold text-on-background truncate">{title}</div>
                            <div className="text-[10px] text-on-surface-variant">{platform}</div>
                          </div>
                          <ChevronRight size={14} className="text-on-surface-variant/50 group-hover:text-primary transition-colors shrink-0" />
                        </div>
                      );
                    })}
                  </div>
          ...
          {/* Career Milestones */}
          <div className="mt-4">
            <h3 className="text-base font-black text-on-background mb-6 flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {milestones.map((m, i) => (
                <div key={i} className={`bg-surface rounded-2xl p-7 border shadow-lg relative overflow-hidden transition-colors duration-300 ${m.unlocked ? 'border-emerald-500/30' : 'border-outline-variant/20 opacity-60'}`}>
                  {m.unlocked && (
                  ...
                  {!m.unlocked && <Lock size={20} className="absolute top-4 right-4 text-on-surface-variant/40" />}
                  <m.icon size={24} className="mb-4" style={{ color: m.unlocked ? m.color : 'var(--color-outline-variant)' }} />
                  <h4 className="text-base font-black text-on-background mb-1">{m.title}</h4>
        ...
        <footer className="px-12 py-5 border-t border-outline-variant/20 flex items-center justify-between opacity-50 select-none">
          <span className="text-[10px] font-logo font-bold text-on-surface-variant uppercase tracking-widest">© 2026 CareerCraft AI • Career Roadmap</span>
          <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">AI Pathing: Multi-Domain Architecture</span>
        </footer>
      </div>
    </div>
  );
