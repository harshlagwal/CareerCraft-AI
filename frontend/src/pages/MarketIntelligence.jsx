    ...
    return (
      <div className="flex h-screen bg-background transition-colors duration-300 overflow-hidden">
        ...
        <div className="flex-1 flex flex-col items-center justify-center p-10">
          ...
          <h2 className="text-2xl font-black text-on-background mb-3">No Data Available</h2>
          <p className="text-on-surface-variant text-sm mb-8 text-center max-w-xs">Run a career analysis first to see market intelligence for your role.</p>
      ...
      <div className="flex-1 flex flex-col h-full overflow-y-auto custom-scrollbar">
        ...
        <main className="flex-1 p-10 lg:p-12 space-y-8">
          ...
          <div>
            ...
            <h1 className="text-4xl font-black text-on-background mb-2">Market Intelligence <span className="text-on-background/30">2026</span></h1>
            <p className="text-sm text-on-surface-variant max-w-2xl leading-relaxed">
              Real-time career trajectories and economic demand mapping for <span className="text-on-background font-bold">{mainRole}</span>. Stay ahead of global workforce shifts.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-8 bg-surface rounded-2xl p-8 border border-outline-variant/30 shadow-xl transition-colors duration-300">
              <div className="flex items-center justify-between mb-6">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center">
                    <BarChart2 size={16} className="text-primary" />
                  </div>
                  <h3 className="text-sm font-black text-on-background uppercase tracking-widest">Salary Benchmark Analysis</h3>
                </div>
                <div className="flex gap-2">
                  <button 
                    onClick={() => setIsSenior(false)}
                    className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest transition-all ${!isSenior ? 'bg-primary/20 border border-primary/40 text-primary' : 'bg-outline-variant/10 border border-outline-variant/20 text-on-surface-variant'}`}>
                    Full Roles
                  </button>
                  <button 
              ...
              <div className="grid grid-cols-2 gap-8 mb-8">
                <div>
                  <div className="text-xs text-on-surface-variant font-bold uppercase tracking-widest mb-1">Global Average (USD)</div>
                  <div className="text-4xl font-black text-on-background">{displayGlobal}</div>
                </div>
                <div>
                  <div className="text-xs text-on-surface-variant font-bold uppercase tracking-widest mb-1">India Benchmark (INR)</div>
              ...
              <div className="text-[10px] text-on-surface-variant font-medium flex items-center justify-between">
            ...
            <div className="lg:col-span-4 flex flex-col gap-6">
              {/* Market Demand */}
              <div className="bg-surface rounded-2xl p-7 border border-outline-variant/30 shadow-xl flex-1 transition-colors duration-300">
                <div className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest mb-4">Market Demand</div>
                ...
                <div className="text-[11px] text-on-surface-variant">88th percentile against all sectors</div>
              </div>

              {/* Annual Growth */}
              <div className="bg-surface rounded-2xl p-7 border border-outline-variant/30 shadow-xl flex-1 transition-colors duration-300">
                <div className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest mb-4">Annual Growth</div>
                <div className="text-5xl font-black text-emerald-400 mb-3">+{mi.growth_rate || '28%'}</div>
                <div className="text-[11px] text-on-surface-variant leading-relaxed">Surging demand in generative AI implementation and cloud architecture.</div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-surface rounded-2xl p-8 border border-outline-variant/30 shadow-xl transition-colors duration-300">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-sm font-black text-on-background uppercase tracking-widest flex items-center gap-3">
            ...
            <div className="flex items-center gap-4 overflow-x-auto pb-2">
              {companies.map((company, i) => (
                <div key={i} className="flex flex-col items-center gap-3 shrink-0 group cursor-pointer">
                  <div className="w-16 h-16 rounded-2xl bg-outline-variant/10 border border-outline-variant/20 flex items-center justify-center group-hover:border-primary/40 transition-all overflow-hidden relative">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-primary/20 to-primary/5 flex items-center justify-center border border-outline-variant/10">
                      <span className="text-lg font-black text-primary">{company.charAt(0)}</span>
                    </div>
                  </div>
                  <div className="text-center">
                    <div className="text-xs font-black text-on-background">{company}</div>
                    <div className="text-[10px] text-on-surface-variant">{OPEN_ROLES[company] || Math.floor(Math.random() * 50 + 10)} Open Roles</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Trending Skills */}
            <div className="bg-surface rounded-2xl p-8 border border-outline-variant/30 shadow-xl transition-colors duration-300">
              <h3 className="text-sm font-black text-on-background uppercase tracking-widest mb-6">Trending Skills</h3>
              <div className="space-y-4">
                {TRENDING_SKILLS_LIST.map((skill, i) => (
                  <div key={i} className="flex items-center justify-between p-4 rounded-xl bg-outline-variant/5 border border-outline-variant/20 group hover:border-primary/40 transition-all">
                    <div className="flex items-center gap-4">
                      <div className="w-8 h-8 rounded-lg bg-primary/10 border border-primary/20 flex items-center justify-center">
                        <skill.icon size={14} className="text-primary" />
                      </div>
                      <div>
                        <div className="text-sm font-bold text-on-background">{skill.name}</div>
                        <div className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest">{skill.growth} increase YoY</div>
                      </div>
                    </div>
            ...
            <div className="bg-surface rounded-2xl p-8 border border-outline-variant/30 shadow-xl transition-colors duration-300">
              <h3 className="text-sm font-black text-on-background uppercase tracking-widest mb-2">Opportunity Hotspots</h3>
              <p className="text-[11px] text-on-surface-variant mb-6">Top growth cities for technology roles</p>
              <div className="space-y-4">
                {HOTSPOTS.map((h) => (
                  <div key={h.rank} className="flex items-center justify-between p-4 rounded-xl bg-outline-variant/5 border border-outline-variant/20">
                    <div className="flex items-center gap-4">
                      <div className="text-[10px] font-black text-primary/50 w-6">{h.rank}</div>
                      <div className="flex items-center gap-2">
                        <MapPin size={14} className="text-on-surface-variant" />
                        <span className="text-sm font-bold text-on-background uppercase tracking-wide">{h.city}</span>
                      </div>
                    </div>
        ...
        <footer className="px-12 py-5 border-t border-outline-variant/20 flex items-center justify-between opacity-50 select-none">
          <span className="text-[10px] font-logo font-bold text-on-surface-variant uppercase tracking-widest">© 2026 CareerCraft AI • Market Intelligence</span>
          <span className="text-[10px] font-bold text-on-surface-variant uppercase tracking-widest">Data: Real-Time Workforce Vectors</span>
        </footer>
      </div>
    </div>
  );
