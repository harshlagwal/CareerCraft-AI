...
const Bullet = ({ text }) => {
  ...
  return (
    <div className="group bg-outline-variant/5 border border-outline-variant/20 rounded-2xl p-6 hover:bg-outline-variant/10 hover:border-primary/30 transition-all relative">
       <p className="text-[13px] text-on-surface-variant leading-relaxed pr-10 group-hover:text-on-background transition-colors">
          {text}
       </p>
       <button 
          onClick={handleCopy}
          className="absolute top-6 right-6 p-2 rounded-lg bg-outline-variant/10 border border-outline-variant/20 text-on-surface-variant hover:text-primary hover:border-primary/20 transition-all opacity-0 group-hover:opacity-100"
       >
...
export default function BulletSuggestions({ suggestions = [] }) {
  ...
  return (
    <div className="bg-surface rounded-[32px] p-10 border border-outline-variant/20 shadow-2xl space-y-8 relative overflow-hidden group transition-colors duration-300">
       ...
       <div className="flex justify-between items-center relative z-10">
          <div className="space-y-2">
             ...
             <p className="text-[10px] text-on-surface-variant font-bold uppercase tracking-widest opacity-60">Generated based on target role vectors</p>
          </div>
